...
Objective C/C/C++使用的编译器前端是 `clang`，`swift`是 swift (https://swift.org/compiler-stdlib/#compiler-architecture)，后端都是 `LLVM`。

![编译器架构](https://user-gold-cdn.xitu.io/2018/12/26/167e865a82e9b53c?w=1240&h=395&f=png&s=30533)
...
LLVM的核心库提供了现代化的 `source-target-independent` 优化器 和支持诸多流行 CPU 架构的代码生成器，这些核心代码是围绕着LLVM IR(中间代码)建立的。

基于LLVM，又衍生出了一些强大的子项目，其中iOS开发者耳熟能详的是：`Clang` 和 `LLDB`。

### clang

clang是C语言家族的编译器前端，诞生之初是为了替代GCC，提供更快的编译速度。一张图了解clang编译的大致流程：

![clang编译流程](https://user-gold-cdn.xitu.io/2018/12/26/167e865a82f1fd8c?w=1240&h=481&f=png&s=82884)
...
LLVM会对生成的IR进行优化，优化会调用相应的Pass进行处理。Pass由多个节点组成，都是 `Pass` 类的子类，每个节点负责做特定的优化，更多细节：`Writing an LLVM Pass`。
...
![XCode编译](https://user-gold-cdn.xitu.io/2018/12/26/167e865a850738e5?w=1240&h=595&f=png&s=430571)
...
![任务依赖关系](https://user-gold-cdn.xitu.io/2018/12/26/167e865a831bfea3?w=1240&h=730&f=png&s=61557)

可以从XCode的`Report Navigator`看到Target的编译顺序：

![Target顺序](https://user-gold-cdn.xitu.io/2018/12/26/167e865a8300d091?w=1240&h=587&f=png&s=242384)
...
![增量编译](https://user-gold-cdn.xitu.io/2018/12/26/167e865a831edef3?w=1240&h=683&f=png&s=60763)
...
![头文件报错](https://user-gold-cdn.xitu.io/2018/12/26/167e865abcf70926?w=1240&h=432&f=png&s=48307)
...
![详细日志](https://user-gold-cdn.xitu.io/2018/12/26/167e865ac28c4579?w=1240&h=558&f=png&s=966247)

把这个日志整体拷贝到命令行中，最后加上`-v`，表示我们希望得到更多的日志信息，执行这段代码，在日志最后可以看到clang是如何找到头文件的：
...
clang发现`#import "TestView.h"`的时候，先在`headermap`(Demo-generated-files.hmap,Demo-project-headers.hmap)里查找，如果headermap文件找不到，接着在own target的framework里找：
...
2. 文件导入后，宏定义容易出现问题。因为是文本导入，并且按照include依次替换，当一个头文件定义了`#define std hello_world`，而第另一个头文件刚好又是C++标准库，那么`include`顺序不同，可能会导致所有的std都会被替换。
...
![swift编译](https://user-gold-cdn.xitu.io/2018/12/26/167e865acc5247f9?w=1084&h=620&f=png&s=32777)
...
![签名](https://user-gold-cdn.xitu.io/2018/12/26/167e865b04c4265a?w=1122&h=528&f=png&s=27779)


Lina收到数据后，用公钥解密签名，得到Leo发过来的摘要；然后自己按照同样的摘要算法计算摘要，如果计算的结果和Leo的一样，说明数据没有被篡改过。

![验证签名](https://user-gold-cdn.xitu.io/2018/12/26/167e865ae8f63582?w=944&h=518&f=png&s=34683)
...
![证书](https://user-gold-cdn.xitu.io/2018/12/26/167e865b2869f951?w=1200&h=684&f=png&s=48492)
...
![代码签名](https://user-gold-cdn.xitu.io/2018/12/26/167e865b4291fac4?w=1240&h=783&f=png&s=597072)
